...
class Virus {
    ...
    _spawner() {
        setTimeout(() => {
            if (!this._spawningEnabled)
                return;
            const x = randomNumber(this._World.Game._canvasBoundary.minX, this._Game._canvasBoundary.maxX + 15);
            const y = randomNumber(this._World.Game._canvasBoundary.minY, this._Game._canvasBoundary.maxY - 13);
            this.spawn(x, y);
...
class World {
    ...
    inceraseDeadZone(count = 10) {
        ...
        this._deadZoneWidth = this._deadZonePixels % this._canvasRect.height;
    }
