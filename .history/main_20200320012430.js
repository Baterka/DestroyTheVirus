...
class Virus {
    ...
    _spawner() {
        setTimeout(() => {
            if (!this._spawningEnabled)
                return;
            const x = randomNumber(this._Game.World.canvasBoundary.minX, this._Game._canvasBoundary.maxX + 15);
            const y = randomNumber(this._Game.World.canvasBoundary.minY, this._Game._canvasBoundary.maxY - 13);
            this.spawn(x, y);
